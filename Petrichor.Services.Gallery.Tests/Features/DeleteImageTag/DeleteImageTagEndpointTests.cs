...
namespace Petrichor.Services.Gallery.Tests.Features.DeleteImageTag;

[Trait("Category", "Integration")]
[Collection(ApiFactoryCollection.Name)]
public class DeleteImageTagEndpointTests(ApiFactory apiFactory)
{
    [Fact]
    public async Task DeleteImageTag_WhenUploader_ReturnsNoContent()
    {
        ...
        addImageTagsResponse.StatusCode.Should().Be(HttpStatusCode.NoContent);
        ...
        var imageWithDeletedTag = await getImageWithDeletedTagResponse.Content.ReadFromJsonAsync<GetImageResponse>();
    ...
    [Fact]
    public async Task DeleteImageTag_WhenAnonymous_ReturnsUnauthorized()
    {
        using var client = apiFactory.CreateClient();

        var response = await client
            .DeleteAsync($"/images/{Guid.NewGuid()}/tags/{Guid.NewGuid()}");
